#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Fact-check ghc-exam1 + ghc-exam2 (196 GH-300/GitHub Copilot questions)
against Gemini's knowledge, batched to keep call count low.

Read-only: writes a report for human review, does NOT touch data.json.
Usage: python tools/gemini_check_gh300.py [--key2|--key3] [--model NAME]
"""
import argparse
import json
import time
...
WORKLIST_FILE = "D:/Work/ai-cert-quiz/tools/_gh300_worklist.json"
REPORT_FILE = "D:/Work/ai-cert-quiz/tools/_gh300_gemini_report.json"
PROGRESS_FILE = "D:/Work/ai-cert-quiz/tools/_gh300_gemini_progress.json"
...
PROMPT_HEADER = """Bạn là chuyên gia chứng chỉ GitHub Copilot (GH-300 / Certified GitHub Copilot exam).
Dưới đây là các câu hỏi trắc nghiệm, mỗi câu đã có đáp án hiện tại được đánh dấu "hiện đang đánh dấu ĐÚNG".
Nhiệm vụ: kiểm tra xem đáp án hiện tại có ĐÚNG theo kiến thức thực tế/tài liệu chính thức về GitHub Copilot không.

Trả lời DUY NHẤT bằng JSON array, mỗi phần tử có dạng:
{"key": "<examId>#<id>", "agree": true/false, "should_be": ["A","B"], "confidence": "high"/"medium"/"low", "reason": "giải thích ngắn gọn bằng tiếng Việt"}
...
- Chỉ đánh false khi bạn thực sự chắc chắn theo tài liệu/hành vi chính thức của GitHub Copilot, không đoán mò.
