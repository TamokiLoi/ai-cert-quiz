...
function Repair-Value($value) {
  ...
  if ($value -is [string]) {
    if ([Text.RegularExpressions.Regex]::IsMatch($value, '\u00C3|\u00C2|\u00E2|\u00C4|\u00C6|\u00F0|\uFFFD')) {
      return [Text.Encoding]::UTF8.GetString($cp1252.GetBytes($value))
    }
    return $value
  }
  ...
  if ($value -is [pscustomobject]) {
    foreach ($property in @($value.PSObject.Properties)) { $value.($property.Name) = Repair-Value $property.Value }
  }
  return $value
}
...
Write-Output 'Applied one encoding repair pass to data.json.'
